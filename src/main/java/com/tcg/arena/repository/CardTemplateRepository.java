...
public interface CardTemplateRepository extends JpaRepository<CardTemplate, Long> {
        ...
        @Query("SELECT c FROM CardTemplate c WHERE c.setCode = :setCode AND (c.cardNumber IS NOT NULL AND c.cardNumber <> 'N/A' AND c.cardNumber <> '')")
        ...
        @Query("SELECT c FROM CardTemplate c WHERE " +
                        ...
                        "(c.cardNumber = :cardNumber OR c.cardNumber LIKE CONCAT(:cardNumber, '/%'))")
